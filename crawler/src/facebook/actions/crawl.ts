...
console.log("config.default.isSearchFacebook", config.default.isSearchFacebook)
  ...
  try {
    ...
    if (config.default.isSearchFacebook) {
      for (const keyword of account.listKeyword) {
        await searchFacebook(context, account, socket, topics, keyword, prisma, null, redisClient)
        try {
          await context.close()
          await browser.close()
        } catch (error) {
    ...
    for (const source of dataSources) {
      ...
      if (source.type != SOURCE_TYPE.FB_GROUP || isJoinedGroup) {
        await getPostsFromSourceMbasic(context, account, source, socket, topics, prisma, null, redisClient)
