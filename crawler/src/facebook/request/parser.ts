import {CrawledFacebookPost} from './types'

export const getPostsFromResponse = (items: any[]): {posts: CrawledFacebookPost[]; nextCursors: string[]} => {
  if (typeof items != 'object') {
    return null
  }
  const posts: CrawledFacebookPost[] = []
  let nextCursors: string[] = []
  for (const item of items) {
    ...
    for (const node of nodes) {
      if (node?.__typename == 'Story' && !node?.comet_sections?.content?.story?.comet_sections?.aggregated_stories) {
        ...
        const id = Buffer.from(node.id, 'base64').toString('utf8')?.split(':').pop()
        // console.log(JSON.stringify(node, null, 2))
        let textContent = node?.comet_sections?.content?.story?.message?.text
        const link = node?.comet_sections?.content?.story?.wwwURL
        let images = []
        let videos = []
        const media = node?.comet_sections?.content?.story?.attachments[0]?.styles?.attachment?.media
        if (media) {
        ...
        const footerAttachment = node?.comet_sections?.content?.story?.attachments[0]?.comet_footer_renderer
        if (footerAttachment) {
          textContent += `\n${footerAttachment.attachment?.title_with_entities?.text}\n${footerAttachment.attachment?.description?.text}`
        }
        const attachments =
          node?.comet_sections?.content?.story?.attachments[0]?.styles?.attachment?.all_subattachments?.nodes
        if (attachments?.length) {
          images.push(
            ...attachments
              .filter((a: any) => a?.media?.__typename == 'Photo')
              .map((a: any) => ({
                id: a?.media?.id,
          ...
          videos.push(
            ...attachments
              .filter((a: any) => a?.media?.__typename == 'Video')
              .map((a: any) => ({
                id: a?.media?.id,
                previewImage: a?.media?.image,
        ...
        const interactions =
          node?.comet_sections?.feedback?.story?.feedback_context?.feedback_target_with_context?.ufi_renderer?.feedback
        ...
        const postedAt = new Date(
          node?.comet_sections?.context_layout?.story?.comet_sections?.metadata?.find(
            (m: any) => m?.story?.creation_time,
          )?.story.creation_time * 1000,
        )
        const isPublic =
          node?.comet_sections?.context_layout?.story?.comet_sections?.metadata?.find(
            (m: any) => m?.story?.privacy_scope?.icon_image?.name,
...
export const normalizeResponse = (res: string) => {
