...
export async function searchYoutube(
  ...
  let keywords = topic.searchKeywords
  ...
  for (const keySearchs of searchList) {
    ...
    try {
      while (countVideo < 100) {
        ...
        if (contents.length >= 10) {
          while (contents.length >= 10) {
            ...
            await filterAndSaveContent(contentProcess, socket, listTopics, null, prisma, "YOUTUBE_SEARCH", producer)
          }
        }
      ...
      if (contents.length > 0) {
        await filterAndSaveContent(contents, socket, listTopics, null, prisma, "YOUTUBE_SEARCH", producer)
