...
;(async () => {
  ...
  while (true) {
    await sleep(10000)
    let accounts = await clientDirectus.request(
      readItems("facebok_accounts", {
        filter: {
          status: {
            _eq: "LIVE",
          },
          lastRunAt: {
            _lte: new Date(new Date().setHours(new Date().getHours() - 4)),
          },
        },
        sort: "lastRunAt",
        limit: 2,
    ...
    const topics = await clientDirectus.request(
      readItems("topics", {
        ...
        sort: "nextSearchFacebookAt",
      }),
    )
    const listSource = await clientDirectus.request(
      readItems("sources", {
        filter: {
          status: {
            _eq: "LIVE",
          ...
          lastCrawledAt: {
            _lte: new Date(new Date().setHours(new Date().getHours() - 2)),
          },
          type: {
            _in: ["FB_ACCOUNT", "FB_GROUP", "FB_PAGE"],
          },
        },
      }),
    )
    ...
    if (listAccounts.length > 0) {
      await Promise.all(
        listAccounts.map(async (account) => {
          const runAcc = await startAccount(clientDirectus, account)
          if (runAcc) {
            ...
            if (browserConfig && account.fbId) {
              ...
              await crawlSingleAccount({browserConfig, account, clientDirectus, socket, redisClient, topics})
            }
          }
        }),
      )
