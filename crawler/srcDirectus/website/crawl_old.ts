...
import {DirectusClient, rest, readItems, readSingleton, StaticTokenClient, RestClient, updateItem} from "@directus/sdk"
...
export async function crawl(
  ...
  try {
    const source = await clientDirectus.request(
      readItems("sources", {
        filter: {
          status: {
            _eq: "LIVE",
    ...
    if (source?.length) {
      if (
        new Date().getTime() - new Date(source[0].lastCrawledAt).getTime() >
        config.default.minTimeCrawlWeb * 60 * 1000
      ) {
        await clientDirectus.request(
          updateItem("sources", source[0].id, {
        ...
        let {link, lastCrawledAt} = source[0]
        const now = new Date()
                      ...
                      let contents = await Promise.all(
                        processLinks.map(async (e: {link: string; postedAt: Date; image: string; video: string}) => {
                          if (await redisClient.get(e.link)) {
                            ...
                            let content = await getMainContent(e.link, false)
        ...
        contents = contents.filter((c) => c?.textContent)
        if (contents.length > 0) {
          await filterAndSaveContent(contents, socket, topics, incognito, clientDirectus, "WEBSITE", source[0])
        }

        if (countCantAccessSitemap === siteMaps.length) {
          const updatedSource = await clientDirectus.request(
            updateItem("sources", source[0].id, {
              count_cant_access: source[0].count_cant_access + 1,
            }),
          )

          if (updatedSource.count_cant_access > 50) {
            await clientDirectus.request(
              updateItem("sources", source[0].id, {
                status: "DEAD",
              }),
    ...
    setTimeout(() => {
      crawl(clientDirectus, socket, incognito, redisClient)
    }, 15 * 1000)
  } catch (error) {
