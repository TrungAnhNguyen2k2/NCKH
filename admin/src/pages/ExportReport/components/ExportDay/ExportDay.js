...
export default function ExportDay({queryDate}) {
  ...
  let start = new Date(queryDate)
  let end = new Date(queryDate)
            ...
            (p, index) =>
              new TableRow({
                children: [
                  ...
                  new TableCell({
                    children: [new Paragraph(moment(p.postedAt).format("HH:mm DD/MM"))],
                    // width: {
                    //   size: 10,
                    //   type: WidthType.PERCENTAGE,
                    // },
                  ...
                  new TableCell({
                    children: [
                      new Paragraph(
                        p?.editedTextContent !== "" ? p?.editedTextContent : p.textContent.substring(0, 300),
                      ),
                    ],
                    // width: {
                    //   size: 40,
        ...
        sections: [
          {
            children: [
              ...
              normalParagraph(
                `Qua nắm bắt tình hình trên không gian mạng trong khoảng thời gian từ 6h ngày ${moment(start).format(
                  "DD/MM/YYYY",
                )} đến 6h ngày ${moment(end).format("DD/MM/YYYY")} có một số tin chính đáng chú ý sau:`,
              ),
