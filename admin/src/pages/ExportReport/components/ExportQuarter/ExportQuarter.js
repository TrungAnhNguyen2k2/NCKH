...
export default function ExportQuarter() {
  ...
  const exportWord = async () => {
    try {
        ...
        sections: [
          {
            children: [
              ...
              new Paragraph({
                children: [
                  new TextRun({
                    text: `(Từ 0h00 ${moment().startOf('quarter').format('DD/M/YYYY')} đến ${moment().format(
                      'hh',
              ...
              new Paragraph({
                children: [
                  new TextRun({
                    text: `Bản tin nổi bật trong qúy`,
                    size: 24,
                  }),
      ...
      addHistory.mutate({newData: {screen: 'Export', description: `Export bản tin quý - word`}, token})

      Packer.toBlob(doc).then((buffer) => {
        saveAs(buffer, 'doctest.docx')
      })
    } catch (error) {
      console.log(error)
    }
  ...
  const exportExcel = async () => {
    let queryStr = `${process.env.REACT_APP_API_URL}/content?userHandle=handledPost&fromDate=${moment()
      .startOf('quarter')
      .toISOString()}&toDate=${moment().endOf('day').toISOString()}`
    exportToCSV(queryStr, 'exceltest')
  }
  const exportToCSV = async (query, fileName) => {
    try {
      ...
      addHistory.mutate({newData: {screen: 'Export', description: `Export bản tin quý - excel`}, token})
  ...
  const exportToPdf = async () => {
    try {
      ...
      addHistory.mutate({newData: {screen: 'Export', description: `Export bản tin quý - pdf`}, token})
