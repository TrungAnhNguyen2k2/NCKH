      ...
      <Document>
        <Page style={styles?.page}>
          <View>
            ...
            <Text style={styles.description1}>{`(Từ 0h00 ${moment(queryDate[0] || new Date()).startOf("day").format("DD/M/YYYY")} đến ${moment(queryDate[1] || new Date()).format("DD/MM/YYYY") == moment(new Date()).format("DD/MM/YYYY") ? `${moment().format("HH")}h${moment().format("mm")}` : "23h59"} ngày ${moment(queryDate[1] || new Date()).format("DD/M/YYYY")})`}</Text>
            <Text style={styles.description2}>{`Qua nắm bắt tính hình trên không gian mạng trong khoảng thời gian từ 0h00 ngày ${moment(queryDate[0] || new Date()).startOf("day").format("DD/M/YYYY")} đến ${moment(queryDate[1] || new Date()).format("DD/MM/YYYY") == moment(new Date()).format("DD/MM/YYYY") ? `${moment().format("HH")}h${moment().format("mm")}` : "23h59"} ${moment(queryDate[1] || new Date()).format("DD/M/YYYY")} có một số tin chính đáng chú ý sau:`}</Text>
          </View>
          <View>
            {tags && tags?.length
              ? tags.map((p, i) => (
          ...
          <View>
            <Text style={{ fontWeight: "bold", marginLeft: 20,marginBottom: 10, marginTop: 10, fontSize: 13 }}>{`Danh sách các bài viết vi phạm từ 0h00 ${moment(queryDate[0] || new Date()).startOf("day").format("DD/M/YYYY")} đến ${moment(queryDate[1] || new Date()).format("DD/MM/YYYY") == moment(new Date()).format("DD/MM/YYYY") ? `${moment().format("HH")}h${moment().format("mm")}` : "23h59"} ngày ${moment(queryDate[1] || new Date()).format("DD/M/YYYY")})`}</Text>
          </View>
          <View style={styles?.table}>
            <View
              style={{
